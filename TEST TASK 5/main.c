...
int animateCar(int xDestination, int yDestination) {
    while (car.x != xDestination * 2 || car.y != yDestination * 2) {
        ...
        if (xDiff > 0 && cityGrid[car.y][car.x + 1] == ' ') {
            updateCar(MOVE_RIGHT);
        } else if (xDiff < 0 && cityGrid[car.y][car.x - 1] == ' ') {
            updateCar(MOVE_LEFT);
        } else if (yDiff > 0 && cityGrid[car.y + 1][car.x] == ' ') {
            updateCar(MOVE_DOWN);
        } else if (yDiff < 0 && cityGrid[car.y - 1][car.x] == ' ') {
            updateCar(MOVE_UP);
        } else if (xDiff > 0 && cityGrid[car.y][car.x - 1] == ' ') {
            updateCar(MOVE_LEFT); // Try moving in the opposite horizontal direction
        } else if (xDiff < 0 && cityGrid[car.y][car.x + 1] == ' ') {
            updateCar(MOVE_RIGHT); // Try moving in the opposite horizontal direction
        } else if (yDiff > 0 && cityGrid[car.y - 1][car.x] == ' ') {
            updateCar(MOVE_UP); // Try moving in the opposite vertical direction
        } else if (yDiff < 0 && cityGrid[car.y + 1][car.x] == ' ') {
            updateCar(MOVE_DOWN); // Try moving in the opposite vertical direction
        } else {
            // If the car can't move in any direction, wait for user input
            printf("Car is stuck. Press Enter to exit...");
